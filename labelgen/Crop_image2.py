...
def cutting(crop_size1 ,crop_size2 ,overlap1 ,overlap2 ,Binary_imagepath ,cuttingpath):#参数介绍：（指定像素宽度,，指定像素高度,指定像素高度,指定宽重叠率,指定高重叠率，输入tif影像矩阵，输出扩充tif矩阵）
    cuttingresult_path = []
    for ii in range(len(Binary_imagepath)):
        ...
        rs = row_num * size1
        '''
        cs = size1 + int(size1 * (1 - overlap1)) * (col_num - 1)
        rs = size2 + int(size2 * (1 - overlap2)) * (row_num - 1)
       ...
       # cuttingresult = [0]
        ...
        for i in range(row_num):    #从高度下手！！！ 可以分成几块！
            for j in range(col_num):
                ...
                #原来的
                file_path = file
                return_file_path = "G:\data\images\\"
                ...
                try:
                    os.remove(file)
                    os.remove(return_file_path +'%09d.png.aux.xml' %(j+1+(i+1)*1000+(ii+1)*1000000))###   I
                except:
                    pass
