...
def cutting(crop_size1 ,crop_size2 ,overlap1 ,overlap2 ,Binary_imagepath ,cuttingpath):#参数介绍：（指定像素宽度,，指定像素高度,指定宽重叠率,指定高重叠率,输入tif影像矩阵，输出扩充tif矩阵）
    cuttingresult_path = []
       ...
       # cuttingresult = [0]
        ...
        for i in range(row_num):    #从高度下手！！！ 可以分成几块！
            for j in range(col_num):
                ...
                b_ysize = min(width - offset_y, size1)
                b_xsize = min(height - offset_x, size2)
                ...
                file_path = file
                return_file_path = "G:\data\labels\\"
                ...
                try:
                    os.remove(file)
                    os.remove(return_file_path +'%09d.png.aux.xml' %(j+1+(i+1)*1000+(ii+1)*1000000))###L
                except:
                    pass
                #os.remove(cuttingresult +'%04d.png.aux.xml' %num)
