...
def cutting(crop_size1 ,crop_size2 ,overlap1 ,overlap2 ,Binary_imagepath ,cuttingpath):#参数介绍：（指定像素宽度,，指定像素高度,输入tif影像矩阵，输出扩充tif矩阵）
    cuttingresult_path = []
    for ii in range(len(Binary_imagepath)):
        ...
        #宽切割多少次运算
        if width - size1 > 0:
            col_num = int((width - size1) / (size1*(1 - overlap1))) + 1
            if (width - size1) % (size1*(1 - overlap1)) != 0:
                col_num += 1
        else:
            col_num = 1
        #高度能切割几份
        if height - size2 > 0:
            row_num = int((height - size2) / (size2*(1 - overlap2))) + 1
            if (height - size2) % (size2*(1 - overlap2)) != 0:
                row_num += 1
        else:
            row_num = 1
        ...
        # out_band11 = binary_erosion(out_band1)
        # out_band12 = binary_erosion(out_band2)
        # out_band13 = binary_erosion(out_band3)
        # out_band11 = binary_erosion(out_band11)
        # out_band12 = binary_erosion(out_band12)
        # out_band13 = binary_erosion(out_band13)
        # out_band11 = binary_erosion(out_band11)
        # out_band12 = binary_erosion(out_band12)
        # out_band13 = binary_erosion(out_band13)
        # out_band11 = binary_erosion(out_band11)
        ...
        # out_band12 = binary_erosion(out_band12)
        # out_band13 = binary_erosion(out_band13)
        # out_band1 = out_band1 - binary_erosion(out_band11) * 255
        # out_band2 = out_band2 - binary_erosion(out_band12) * 255
        # out_band3 = out_band3 - binary_erosion(out_band13) * 255
        out_band11 = out_band1
        out_band21 = out_band2
        out_band31 = out_band3
        for i in range(3):
            out_band1 =  binary_erosion(out_band1) * 255
            out_band2 =  binary_erosion(out_band2) * 255
            out_band3 =  binary_erosion(out_band3) * 255
        out_band1 = out_band11 - out_band1
        out_band2 = out_band21 - out_band2
        out_band3 = out_band31 - out_band3
        #d多线程
        '''
        thread_list = []
        new1 = []
        ...
        cs = size1 + int(size1 * (1 - overlap1)) * (col_num - 1) #宽
        rs = size2 + int(size2 * (1 - overlap2)) * (row_num - 1) #高
        ...
        # 创建切出来的要存的文件
        out_ds = gtif_driver.Create(file, cs, rs, outbandsize, datatype)  #（文件路径、宽、高、波段数、数据类型）
        # 获取原图的原点坐标信息
       ...
       # cuttingresult = [0]
        ...
        for i in range(row_num):    #从高度下手！！！ 可以分成几块！
            for j in range(col_num):
                offset_x = i * int(size2 * (1 - overlap2))
                offset_y = j * int(size1 * (1 - overlap1))
                ...
                return_file_path = "D:\DeepLeaningDataspredict\\膨胀空心label1\\"
