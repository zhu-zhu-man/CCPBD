...
for ii in range(3): #尝试打开文件
    try:
        ...
        tif_path = np.load(datapath + '\\tif_path.npy')[:21]
        #output_path = np.load(datapath + '\\output_path.npy')
        expend_path = np.load(datapath + '\\expend_path.npy')[:21]
...
overlap1 = 0.5  #膨胀率
overlap2 = 0.5  #膨胀率
AA = Crop_image膨胀predict.cutting(cutw ,cuth ,overlap1 ,overlap2 ,tif_path ,expend_path)#切割遥感影像，小块路径返回
del tif_path,expend_path
np.save(datapath + '\\切割完成影响result_膨胀predict.npy',AA)
